...
public class ContactAdapter extends ArrayAdapter<Contact> {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
    	...
    	try {
			...
			if (position % 2 == 0) {
				id = adapterContext.getResources().getColor(R.color.system_red);
			} else {
				id = adapterContext.getResources().getColor(R.color.system_blue);
			}
			contactName.setTextColor(id); //  red or blue
			// Q6.1
        	contactNumber.setText("Home:"+ contact.getPhoneNumber()+ " Cell:"+ contact.getCellNumber());
			// Q6.2
			contactAddress.setText(replaceNull(contact.getStreetAddress()) +","+ replaceNull(contact.getCity()) + ","+
					replaceNull(contact.getState())+ ","+ replaceNull(contact.getZipCode()));
